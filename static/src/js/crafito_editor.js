odoo.define('theme_eagleshop.eagleshop_editor_js', function(require) {
    ...
    ajax.loadXML('/theme_eagleshop/static/src/xml/theme_eagleshop.xml', qweb);
    ...
    options.registry.theme_eagleshop_blog_custom_snippet = options.Class.extend({
        start: function(editMode) {
            ...
            if (!editMode) {
                self.$el.find(".eagleshop_blog_slider").on("click", _.bind(self.theme_eagleshop_blog_slider, self));
            }
        },
        onBuilt: function() {
            var self = this;
            this._super();
            if (this.theme_eagleshop_blog_slider()) {
                this.theme_eagleshop_blog_slider().fail(function() {
        ...
        theme_eagleshop_blog_slider: function(type, value) {
            var self = this;
            if (type != undefined && type.type == "click" || type == undefined) {
                self.$modal = $(qweb.render("theme_eagleshop.eagleshop_blog_slider_block"));
                ...
                ajax.jsonRpc('/theme_eagleshop/blog_get_options', 'call', {}).done(function(res) {
    ...
    options.registry.theme_eagleshop_multi_cat_custom_snippet = options.Class.extend({
        ...
        multi_category_slider: function(type, value) {
            var self = this;
            if (type != undefined && type.type == "click" || type == undefined) {
                self.$modal = $(qweb.render("theme_eagleshop.multi_product_custom_slider_block"));
                self.$modal.appendTo('body');
                self.$modal.modal();
                ...
                ajax.jsonRpc('/theme_eagleshop/product_multi_get_options', 'call', {}).done(function(res) {
    ...
    options.registry.theme_eagleshop_coming_soon_mode_one = options.Class.extend({
        start: function(editMode) {
            var self = this;
            this._super();
            this.$target.removeClass("o_hidden");
            ...
            if (!editMode) {
                self.$el.find(".biztech_coming_soon_mode_one").on("click", _.bind(self.theme_eagleshop_coming_soon_mode_one_function, self));
            }
        },
        ...
        onBuilt: function() {
            var self = this;
            this._super();
            if (this.theme_eagleshop_coming_soon_mode_one_function()) {
                self.theme_eagleshop_coming_soon_mode_one_function.fail(function() {
                    self.getParent()._removeSnippet();
                });
            }
        },
        ...
        theme_eagleshop_coming_soon_mode_one_function: function(type, value) {
            if (type != undefined && type.type == "click" || type == undefined) {
                var self = this;
                self.$modal = $(qweb.render("theme_eagleshop.model_dialog_commingsoon"));
                self.$modal.appendTo('body');
                self.$modal.modal();
    ...
    options.registry.theme_eagleshop_coming_soon_mode_two = options.Class.extend({
        start: function(editMode) {
            var self = this;
            this._super();
            this.$target.removeClass("o_hidden");
            ...
            if (!editMode) {
                self.$el.find(".biztech_coming_soon_mode_two").on("click", _.bind(self.theme_eagleshop_coming_soon_mode_two_function, self));
            }
        },
        ...
        onBuilt: function() {
            var self = this;
            this._super();
            if (this.theme_eagleshop_coming_soon_mode_two_function()) {
                self.theme_eagleshop_coming_soon_mode_two_function.fail(function() {
                    self.getParent()._removeSnippet();
                });
            }
        },
        ...
        theme_eagleshop_coming_soon_mode_two_function: function(type, value) {
            if (type != undefined && type.type == "click" || type == undefined) {
                var self = this;
                self.$modal = $(qweb.render("theme_eagleshop.model_dialog_commingsoon"));
                self.$modal.appendTo('body');
    ...
    options.registry.theme_eagleshop_category_slider = options.Class.extend({
        start: function(editMode) {
            var self = this;
            this._super();
            this.$target.removeClass("o_hidden");
        ...
        cat_slider: function(type, value) {
            var self = this;
            if (type != undefined && type.type == "click" || type == undefined) {
                self.$modal = $(qweb.render("theme_eagleshop.eagleshop_dynamic_category_slider"));
                self.$modal.appendTo('body');
                self.$modal.modal();
                var $slider_type = self.$modal.find("#slider_type"),
                ...
                ajax.jsonRpc('/theme_eagleshop/category_get_options', 'call', {}).done(function(res) {
    ...
    options.registry.theme_eagleshop_product_slider = options.Class.extend({
        start: function(editMode) {
            var self = this;
            this._super();
            this.$target.removeClass("o_hidden");
        ...
        prod_slider: function(type, value) {
            var self = this;
            if (type != undefined && type.type == "click" || type == undefined) {
                self.$modal = $(qweb.render("theme_eagleshop.eagleshop_dynamic_product_slider"));
                self.$modal.appendTo('body');
                self.$modal.modal();
                var $slider_type = self.$modal.find("#slider_type"),
                ...
                ajax.jsonRpc('/theme_eagleshop/product_get_options', 'call', {}).done(function(res) {
                    $('#slider_type option[value!="0"]').remove();
                    _.each(res, function(y) {
                        $("select[id='slider_type'").append($('<option>', {
                            value: y["id"],
    ...
    options.registry.theme_eagleshop_featured_product_slider = options.Class.extend({
        start: function(editMode) {
            var self = this;
            this._super();
            this.$target.removeClass("o_hidden");
        ...
        featured_prod_slider: function(type, value) {
            var self = this;
            if (type != undefined && type.type == "click" || type == undefined) {
                self.$modal = $(qweb.render("theme_eagleshop.eagleshop_dynamic_featured_product_slider"));
                self.$modal.appendTo('body');
                self.$modal.modal();
                ...
                ajax.jsonRpc('/theme_eagleshop/featured_product_get_options', 'call', {}).done(function(res) {
                    $('#slider_type option[value!="0"]').remove();
                    _.each(res, function(y) {
    ...
    options.registry.biztech_new_theme_fact_sheet_jsselector = options.Class.extend({
        ...
        biztech_fact_sheet_function: function(type) {
            if (type != undefined && type.type == "click" || type == undefined) {
                var self = this;
                var factcount
                self.$modal = $(qweb.render("theme_eagleshop.eagleshop_dynami_fact_sheet"));
                self.$modal.appendTo('body');
                self.$modal.modal();
    ...
    options.registry.biztech_new_theme_skill_jsselector = options.Class.extend({
        ...
        biztech_new_theme_skill_function: function(type) {
            if (type != undefined && type.type == "click" || type == undefined) {
                var self = this;
                var skillcount;
                self.$modal = $(qweb.render("theme_eagleshop.eagleshop_dyanamic_skill"));
                self.$modal.appendTo('body');
                self.$modal.modal();
    ...
    options.registry.theme_eagleshop_news1_js = options.Class.extend({
        start: function(editMode) {
            var self = this;
            this._super();
            ...
            if (!editMode) {
                self.$el.find(".eagleshop_newsone_slider").on("click", _.bind(self.theme_eagleshop_newsone_slider, self));
            }
        },
        onBuilt: function() {
            var self = this;
            this._super();
            if (this.theme_eagleshop_newsone_slider()) {
                this.theme_eagleshop_newsone_slider().fail(function() {
                    self.getParent()._removeSnippet();
                });
            }
        },
        ...
        theme_eagleshop_newsone_slider: function(type, value) {
            var self = this;
            if (type != undefined && type.type == "click" || type == undefined) {
                self.$modal = $(qweb.render("theme_eagleshop.eagleshop_blog_slider_block"));
                self.$modal.appendTo('body');
    ...
    options.registry.theme_eagleshop_news2_js = options.Class.extend({
        start: function(editMode) {
            var self = this;
            this._super();
            this.$target.removeClass("o_hidden");
            ...
            if (!editMode) {
                self.$el.find(".eagleshop_newstwo_slider").on("click", _.bind(self.theme_eagleshop_newstwo_slider, self));
            }
        },
        onBuilt: function() {
            var self = this;
            this._super();
            if (this.theme_eagleshop_newstwo_slider()) {
                this.theme_eagleshop_newstwo_slider().fail(function() {
                    self.getParent()._removeSnippet();
                });
            }
        },
        ...
        theme_eagleshop_newstwo_slider: function(type, value) {
            var self = this;
            if (type != undefined && type.type == "click" || type == undefined) {
                self.$modal = $(qweb.render("theme_eagleshop.eagleshop_blog_slider_block"));
                self.$modal.appendTo('body');
    ...
    options.registry.theme_eagleshop_hardware_blog_snippet = options.Class.extend({
        start: function(editMode) {
            var self = this;
            this._super();
            this.$target.removeClass("o_hidden");
            this.$target.find('.theme_eagleshop_new_blog').empty();
            if (!editMode) {
                self.$el.find(".theme_eagleshop_new_blog").on("click", _.bind(self.theme_eagleshop_new_blog_function, self));
            }
        },
        onBuilt: function() {
            var self = this;
            this._super();
            if (this.theme_eagleshop_new_blog_function()) {
                this.theme_eagleshop_new_blog_function().fail(function() {
                    self.getParent()._removeSnippet();
                });
            }
        },
        ...
        cleanForSave: function() {
            $('.theme_eagleshop_new_blog').empty();
        },

        theme_eagleshop_new_blog_function: function(type, value) {
            var self = this;
            if (type != undefined && type.type == "click" || type == undefined) {
                self.$modal = $(qweb.render("theme_eagleshop.eagleshop_blog_slider_block"));
                self.$modal.appendTo('body');
    ...
    options.registry.theme_eagleshop_faq_three_tabing = options.Class.extend({
        start: function() {
            var self = this;
    ...
    options.registry.theme_eagleshop_faq_one_tabing = options.Class.extend({
        start: function() {
            var self = this;
